...
**How to apply:** Keep the visibility flag and step progress in the authenticated user payload, use an authenticated server mutation for every action, and refresh the in-memory auth user from the server response.